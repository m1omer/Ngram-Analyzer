...
export default function Predict() {
  ...
  const handlePredict = async () => {
    ...
    setPrediction(null);
    try {
      const predictRes = await axios.post("http://localhost:8000/predict", new URLSearchParams({ context }));
      const { predicted_word, probability } = predictRes.data;

      let surprisal_bits = null;
      try {
        const surprisalRes = await axios.post("http://localhost:8000/surprisal", new URLSearchParams({
          context,
          word: predicted_word
        }));
